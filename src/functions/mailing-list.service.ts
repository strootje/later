...
import { getUserMiddleware, updateUser } from "#/functions/user.service.ts";
...
const getSubscriberMiddleware = createMiddleware()
...
export const getSubscriber = createServerFn()
  .middleware([getSubscriberMiddleware])
  .handler(({ context: { subscriber } }) => {
    return subscriber;
  });
...
export const updateSubscriber = createServerFn()
  ...
  .handler(async ({ context: { subscriber }, data: { listId, subscribe } }) => {
...
export const getMailingLists = createServerFn()
  .handler(async () => {
